                ...
                <div class="row column_title">
                    <div class="col-md-12">
                        <div class="page_title">
                            <a href="{{ route('university.merti.create') }}" class="btn btn-info float-left">Add merit</a>&nbsp;
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-12 grid-margin stretch-card">
                        <h4 style="font-size:150%;" class="card-title">Marit List </h4>
...
<script src="{{ asset('Admin/asset/js/sweetalert.min.js') }}"></script>
    ...
    $(document).on('click', '#status', function() {
        ...
        , }).then((willDelete) => {
            if (willDelete) {
                $.ajax({
                    url: "{{ route('university.merti_status') }}"
                    , type: "get"
                    , data: {
                        id: id
                    , }
                    ...
                    , success: function(data) {
                        if (data) {
                            ...
                            window.LaravelDataTables["merit-table"].draw();
                        }
                    }
                });
            } else {
